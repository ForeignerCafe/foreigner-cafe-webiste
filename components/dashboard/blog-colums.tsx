  ...
  },
 {
  ...
  cell: ({ row }) => (
    <div className="relative w-10 h-10 overflow-hidden" style={{ borderRadius: "30%" }}>
      {typeof row.getValue("mainImage") === "string" && row.getValue("mainImage") ? (
        <Image
          src={
            row.getValue("mainImage").startsWith("blob:")
              ? "/placeholder.svg"
...

  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className={`px-2 py-1 text-sm rounded-full w-20 text-center ${statusStyles[status] || ""}`}>
          {status}
        </div>
      );
    },
  ...
  router.push(`/admin/add-blog?id=${blog.slug}`);
};
        ...
        <>
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => { setBlogToDelete(blog); setIsDeleteModalOpen(true); }}>
